...
- OpenAI: chat completion streaming chunk size can now be configured to
  control the rate of streaming callbacks. OpenAI streams single tokens back. This
  can create a very high rate of streaming callbacks. To reduce this, Venice
  can aggregate streaming tokens to strings of a minimal length.
...
- Fixed a flaw in the custom options processing for the launcher that has been introduced 
  with the last version. The custom options '-dir' and '-minimal' have not been passed 
  downstream to the executed script, should they have been used (all other custom options 
  are fine).
...
- [#11](https://github.com/jlangch/venice/issues/11): Support *Hybrid Encryption* for
...
- Fixed Venice launcher edge case with the `-file` option that was accidentally passed down to the script
