...
public class MathFunctions {
	...
	public static VncFunction long_ = 
		new VncFunction(
				"long", 
				VncFunction
					.meta()
					.arglists("(long x)")		
					.doc("Converts x to long")
					.examples("(long 10.23)", "(long 10.23M)", "(long \"10\")")
					.build()
		) {	
			public VncVal apply(final VncList args) {
				...
				if (Types.isVncLong(arg)) {
					...
					return new VncLong(((VncDouble)arg).getValue().longValue());
				}
				else if (Types.isVncBigDecimal(arg)) {
					return new VncLong(
								((VncBigDecimal)args.first()).getValue().longValue());
				}
				else if (Types.isVncString(arg)) {
					final String val = ((VncString)args.first()).getValue();
					try {
						return new VncLong(Long.parseLong(val));
					}
					catch(Exception ex) {
						throw new VncException(String.format(
								"Cannot convert string '%s' to long", val));
					}
				}
				else {
					throw new VncException(String.format(
							"Invalid argument type %s while calling function 'long'",
							Types.getClassName(arg)));
				}
			}
	...
	public static VncFunction double_ = 
		new VncFunction(
				"double", 
				VncFunction
					.meta()
					.arglists("(double x)")		
					.doc("Converts x to double")
					.examples("(double 10)", "(double 10.23M)", "(double \"10.23\")")
					.build()
		) {	
			public VncVal apply(final VncList args) {
				...
				if (Types.isVncLong(arg)) {
					return new VncDouble(((VncLong)arg).getValue().doubleValue());
				}
				else if (Types.isVncDouble(arg)) {
					return arg;
				}
					...
					return new VncDouble(
								((VncBigDecimal)args.first()).getValue().doubleValue());
				}
				else if (Types.isVncString(arg)) {
					final String val = ((VncString)args.first()).getValue();
					try {
						return new VncDouble(Double.parseDouble(val));
					}
					catch(Exception ex) {
						throw new VncException(String.format(
								"Cannot convert string '%s' to double", val));
					}
				}
				else {
					throw new VncException(String.format(
							"Invalid argument type %s while calling function 'double'",
							Types.getClassName(arg)));
				}
			}
	...
	public static VncFunction decimal_ = 
		new VncFunction(
				"decimal", 
				VncFunction
					.meta()
					.arglists("(decimal x scale rounding-mode)")		
					.doc(
						"Converts x to decimal. rounding-mode is one of (:CEILING, :DOWN, " + 
						":FLOOR, :HALF_DOWN, :HALF_EVEN, :HALF_UP, :UNNECESSARY, :UP)")
					.examples(
						"(decimal 10 2 :HALF_UP)", 
						"(decimal 10.23 2 :HALF_UP)", 
						"(decimal 10.23456M 2 :HALF_UP)", 
						"(decimal \"10.23\" 2 :HALF_UP)")
					.build()
		) {	
			public VncVal apply(final VncList args) {
				assertArity("decimal", args, 3); 
							
				final VncVal arg = args.first();
				final VncLong scale = Coerce.toVncLong(args.second());
				final RoundingMode roundingMode = VncBigDecimal.toRoundingMode(Coerce.toVncString(args.third()));
								
				if (Types.isVncLong(arg)) {
					return new VncBigDecimal(
								new BigDecimal(((VncLong)arg).getValue())
										.setScale(scale.getValue().intValue(), roundingMode));
				}
				else if (Types.isVncDouble(arg)) {
					return new VncBigDecimal(
								new BigDecimal(((VncDouble)arg).getValue())
										.setScale(scale.getValue().intValue(), roundingMode));
				}
				else if (Types.isVncBigDecimal(arg)) {
					return new VncBigDecimal(
					...
					try {
						...
						throw new VncException(String.format(
								"Cannor convert string '%s' to decimal", val));
					}
				}
				else {
					throw new VncException(String.format(
							"Invalid argument type %s while calling function 'decimal'",
							Types.getClassName(arg)));
				}
			}
