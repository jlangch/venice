...
public class VncCircularBuffer extends VncCollection implements VncMutable, Iterable<VncVal> {

    public VncCircularBuffer(final int capacity) {
        super(Constants.Nil);
        this.capacity = capacity;
        this.queue = new ConcurrentEvictingQueue<>(capacity);
    }

    private VncCircularBuffer(final VncCircularBuffer queue, final VncVal meta) {
        super(meta);
    ...
    public VncCollection emptyWithMeta() {
        return new VncCircularBuffer(capacity);
    }

    @Override
    public VncCircularBuffer withMeta(final VncVal meta) {
        return new VncCircularBuffer(this, meta);
    }
    ...
    public TypeRank typeRank() {
        return TypeRank.CIRCULAR_BUFFER;
    }
    ...
    public static final String TYPE = ":core/circular-buffer";

    private static final long serialVersionUID = -564531670922145260L;

    private final int capacity;
    private final ConcurrentEvictingQueue<VncVal> queue;
}
