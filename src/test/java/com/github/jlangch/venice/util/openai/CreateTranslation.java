...
public final class CreateTranslation {
    ...
    public static void main(String[] args)  throws Exception {
        ...
        TranslationCreateParams paramsTranslate = TranslationCreateParams.builder()
            .file(audio)
            // many more parameters
            .model(AudioModel.GPT_4O_TRANSCRIBE)
            .build();

        TranslationCreateResponse response = client.audio().translations().create(paramsTranslate);


        Translation translation = response.asTranslation();
        if (translation == null) {
