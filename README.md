...
Start the REPL with `java -jar venice-1.1.0.jar`
...
```text
foo> java -jar venice-1.1.0.jar -script "(+ 1 1)"
=> 2
```

```text
foo> echo "(+ 1 1)" > script.venice
foo> java -jar venice-1.1.0.jar -file script.venice
=> 2
```
...
```groovy
dependencies {
    compile 'com.github.jlangch:venice:1.1.0'
}
```
