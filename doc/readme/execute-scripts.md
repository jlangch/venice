...
```text
foo> java -jar venice-1.9.13.jar -script "(+ 1 1)"
=> 2
```
...
```text
foo> java -jar venice-1.9.13.jar -file script.venice
=> 19.634954084936208
```
...
```text
foo> java -jar venice-1.9.13.jar -file script.venice 3
=> 4
```
...
foo> java -jar bin/venice-1.9.13.jar -file script.venice -loadpath "/users/foo/venice/scripts"
```
...
```text
foo> java -jar venice-1.9.13.jar -script "(time (reduce + (map (fn [x] (cond (< x 0) -1 (> x 0) 1 :else 0)) (range -10000 10001))))"
...
```text
foo> java -jar venice-1.9.13.jar -macroexpand -script "(time (reduce + (map (fn [x] (cond (< x 0) -1 (> x 0) 1 :else 0)) (range -10000 10001))))"
